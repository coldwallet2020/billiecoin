UNIX BUILD NOTES
====================
Some notes on how to build Billiecoin Core in Unix.
...
Note
---------------------
Always use absolute paths to configure and compile billiecoin and the dependencies,
...
This will build billiecoin-qt as well if the dependencies are met.
...
memory available when compiling Billiecoin Core. On systems with less, gcc can be
...
`--with-incompatible-bdb` to configure. See the section "Disable-wallet mode" to build Billiecoin 
Core without wallet.
...
>**Ubuntu:** db4.8 packages are available [here](https://launchpad.net/~billiecoin/+archive/billiecoin).
...
If you want to build Billiecoin-Qt, make sure that the required packages for Qt development are 
...
Once these are installed, they will be found by configure and a billiecoin-qt executable will be
built by default.
...
The version of Boost that ships with CentOS is too old to work with Billiecoin - version 1.55+ 
...
to your `.bashrc` as well to configure this variable on when starting `billiecoind`.
...
Notes
-----
The release is built with GCC and then "strip billiecoind" to strip the debug symbols, 
...
so from the root of the Billiecoin repository.
...
Security
--------
To help make your billiecoin installation more secure by making certain attacks impossible to
...
* Non-executable Stack
  ...
  vulnerable buffers are found. By default, billiecoin should be built with a non-executable stack
...
Disable-wallet mode
--------------------
When the intention is to run only a P2P node without a wallet, billiecoin may be compiled in 
...
[PKGBUILD](https://projects.archlinux.org/svntogit/community.git/tree/billiecoin/trunk/PKGBUILD). 
As mentioned above, when maintaining portability of the wallet between the standard Billiecoin 
