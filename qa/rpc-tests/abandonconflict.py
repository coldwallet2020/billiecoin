...
class AbandonConflictTest(BilliecoinTestFramework):
    ...
    def run_test(self):
        ...
        inputs.append({"txid":txA, "vout":nA})
        outputs = {}
        outputs[self.nodes[1].getnewaddress()] = Decimal("9.5369")
        ...
        # Verify that B and C's 10 BILCO outputs are available for spending again because AB1 is now conflicted
        newbalance = self.nodes[0].getbalance()
        assert(newbalance == balance + Decimal("20"))
        balance = newbalance

        # There is currently a minor bug around this and so this test doesn't work.  See Issue #7315
        # Invalidate the block with the double spend and B's 10 BILCO output should no longer be available
