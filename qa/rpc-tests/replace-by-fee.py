...
def make_utxo(node, amount, confirmed=True, scriptPubKey=CScript([1])):
    ...
    """
    fee = 1*COIN
    while node.getbalance() < finnea_round((amount + fee)/COIN):
        node.generate(100)
        #print (node.getbalance(), amount, fee)

    new_addr = node.getnewaddress()
    #print new_addr
    txid = node.sendtoaddress(new_addr, finnea_round((amount+fee)/COIN))
...
class ReplaceByFeeTest(BilliecoinTestFramework):
    ...
    def test_doublespend_chain(self):
        ...
        # child fees - 40 BILCO - so this attempt is rejected.
        dbl_tx = CTransaction()
