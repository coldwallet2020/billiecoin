...
###   billiecoincore.org and billiecoin.green and compares them.
...
WORKINGDIR="/tmp/billiecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://billiecoincore.org"
HOST2="https://billiecoin.green"
BASEDIR="/bin/"
VERSIONPREFIX="billiecoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'billiecoin-',
...
if [ $? -ne 0 ]; then
   echo "billiecoin.green failed to provide signature file, but billiecoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "billiecoin.green and billiecoincore.org signature files were not equal?"
