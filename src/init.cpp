 ...
 * between billiecoind, and billiecoin-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("billiecoin-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), BILLIECOIN_CONF_FILENAME));
    if (mode == HMM_BILLIECOIND)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), BILLIECOIN_PID_FILENAME));
#endif
    ...
    std::string debugCategories = "addrman, alert, bench, cmpctblock, coindb, db, http, leveldb, libevent, lock, mempool, mempoolrej, net, proxy, prune, rand, reindex, rpc, selectcoins, tor, zmq, "
                                  "billiecoin (or specifically: gobject, instantsend, keepass, masternode, mnpayments, mnsync, privatesend, spork)"; // Don't translate these and qt below
    if (mode == HMM_BILLIECOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Billiecoin specific functionality (Masternodes, PrivateSend, InstantSend, Governance) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-privatesendamount=<n>", strprintf(_("Keep N BILCO anonymized (%u-%u, default: %u)"), MIN_PRIVATESEND_AMOUNT, MAX_PRIVATESEND_AMOUNT, DEFAULT_PRIVATESEND_AMOUNT));
...
std::string LicenseInfo()
{
	...
	return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Dash and Syscoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
		FormatParagraph(strprintf(_("Copyright (C) 2018-%i The Billiecoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
		"\n" +
		...
		"\n" +
		FormatParagraph(_("Billiecoin is open source software produced by a global network of developers. By downloading, distributing and using Billiecoin and the Billiecoin network you release the developers involved in the Billiecoin Project past, present, and future from any and all liability. You are responsible for your creations on the Billiecoin network. You agree that the developers of the Billiecoin Project carry no responsibility for the actions/data or entities of *any* definition created on the network by yourself or others on the network to which you may be exposed.")) + "\n" +
		"\n" +
		FormatParagraph(_("The developers of the Billiecoin Project do not have the power to modify data on the Billiecoin network, it is backed by an immutable blockchain, which you further acknowledge through use of Billiecoin, the Billiecoin network, and Billiecoin services. If you do not agree to these terms, please refrain from using Billiecoin and its related services.")) +
		"\n";
}
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("billiecoin-loadblk");
...
/** Sanity checks
 *  Ensure that Billiecoin Core is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Billiecoin Core version %s\n", FormatFullVersion());
}
...
bool AppInitParameterInteraction()
{
    ...
    // 1-finnea-fee transactions. It should be set above the real
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Billiecoin Core process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", BILLIECOIN_CONF_FILENAME)).string());
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
				...
				// BILLIECOIN Check for changed -addressindex state
    ...
    //lite mode disables all Billiecoin-specific functionality
    ...
    if((!fLiteMode && fTxIndex == false)
       && chainparams.NetworkIDString() != CBaseChainParams::REGTEST) { // TODO remove this when pruning is fixed. See https://github.com/billiecoinpay/billiecoin/pull/1817 and https://github.com/billiecoinpay/billiecoin/pull/1743
    ...
    if(fMasternodeMode) {
        ...
        if(!strMasterNodePrivKey.empty()) {
            ...
            LogPrintf("  pubKeyMasternode: %s\n", CBilliecoinAddress(activeMasternode.pubKeyMasternode.GetID()).ToString());
        } else {
    ...
    // ********************************************************* Step 11c: update block tip in Billiecoin modules
    ...
    // ********************************************************* Step 11d: schedule Billiecoin-specific tasks
