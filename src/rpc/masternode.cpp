...
UniValue masternode(const JSONRPCRequest& request)
{
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.outpoint.ToStringShort()));
        obj.push_back(Pair("payee",         CBilliecoinAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "genkey")
    {
        ...
        return CBilliecoinSecret(secret).ToString();
    }
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CBilliecoinAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
...
UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "sentinel" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print Billiecoin address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (const auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CBilliecoinAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                objMN.push_back(Pair("payee", CBilliecoinAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
                ...
                CBilliecoinAddress address(mn.pubKeyCollateralAddress.GetID());
    ...
    if (strCommand == "decode")
    {
        ...
        for (const auto& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CBilliecoinAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CBilliecoinAddress(mnb.pubKeyMasternode.GetID()).ToString()));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    { "billiecoin",               "masternode",             &masternode,             true,  {} },
    { "billiecoin",               "masternodelist",         &masternodelist,         true,  {} },
    { "billiecoin",               "masternodebroadcast",    &masternodebroadcast,    true,  {} },
    { "billiecoin",               "getpoolinfo",            &getpoolinfo,            true,  {} },
    { "billiecoin",               "sentinelping",           &sentinelping,           true,  {} },
#ifdef ENABLE_WALLET
    { "billiecoin",               "privatesend",            &privatesend,            false, {} },
#endif // ENABLE_WALLET
};
