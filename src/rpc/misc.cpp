...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total billiecoin balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized billiecoin balance of the wallet\n"
	...
	obj.push_back(Pair("version", BILLIECOIN_CLIENT_VERSION));
	// BILLIECOIN
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "billiecoin|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use '+' to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "billiecoin")
            + HelpExampleRpc("debug", "billiecoin+net")
        );
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            BOOST_FOREACH(const CTxDestination& addr, addresses)
                a.push_back(CBilliecoinAddress(addr).ToString());
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given billiecoin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The billiecoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
			"  \"address\" : \"billiecoinaddress\", (string) The billiecoin address validated\n"
			// BILLIECOIN
			"  \"zaddress\" : \"billiecoinaddress\", (string) The zcash t-addr associated with this billiecoin address validated\n"
			"  \"saddress\" : \"billiecoinaddress\", (string) The Billiecoin 3 S address scheme associated with this billiecoin address validated\n"
			"  \"btcaddress\" : \"billiecoinaddress\", (string) The bitcoin address associated with this billiecoin address validated\n"
			"  \"alias\" : \"billiecoinalias\", (string) The billiecoin alias associated with this billiecoin address validated\n"
...
#ifdef ENABLE_WALLET
		// Case 1: Billiecoin address and we have full public key:
		CBilliecoinAddress address(ks);
		// BILLIECOIN
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are billiecoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) billiecoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The billiecoin address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CBilliecoinAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CBilliecoinAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
{
    if (params[0].isStr()) {
        CBilliecoinAddress address(params[0].get_str());
        uint160 hashBytes;
        int type = 0;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CBilliecoinAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
UniValue getaddressmempool(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"finneas\"  (number) The difference of finneas\n"
    ...
    for (std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> >::iterator it = indexes.begin();
        ...
        delta.push_back(Pair("finneas", it->second.amount));
...
UniValue getaddressutxos(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "    \"finneas\"  (number) The number of finneas of the output\n"
    ...
    for (std::vector<std::pair<CAddressUnspentKey, CAddressUnspentValue> >::const_iterator it=unspentOutputs.begin(); it!=unspentOutputs.end(); it++) {
        ...
        output.push_back(Pair("finneas", it->second.finneas));
...
UniValue getaddressdeltas(const JSONRPCRequest& request)
{
    ...
    for (std::vector<std::pair<CAddressIndexKey, CAmount> >::const_iterator it=addressIndex.begin(); it!=addressIndex.end(); it++) {
        ...
        UniValue delta(UniValue::VOBJ);
        delta.push_back(Pair("finneas", it->second));
        delta.push_back(Pair("txid", it->first.txhash.GetHex()));
        delta.push_back(Pair("index", (int)it->first.index));
...
UniValue getaddressbalance(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"balance\"  (string) The current balance in finneas\n"
            "  \"received\"  (string) The total number of finneas received (including change)\n"
            "}\n"
            "\nExamples:\n"
...
UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "billiecoin-cli and the GUI. There is no server-side difference."
        );
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* Billiecoin features */
    { "billiecoin",               "mnsync",                 &mnsync,                 true,  {} },
    { "billiecoin",               "spork",                  &spork,                  true,  {"value"} },
