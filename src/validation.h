...
#ifndef BILLIECOIN_VALIDATION_H
#define BILLIECOIN_VALIDATION_H
...
//! Discourage users to set fees higher than this amount (in finneas) per kB
static const CAmount HIGH_TX_FEE_PER_KB = 0.1 * COIN;
//! -maxtxfee will warn if called with a higher fee than this amount (in finneas)
...
static const int64_t DEFAULT_MAX_TIP_AGE = 2.5 * 6 * 60 * 60; // ~144 blocks behind -> 2 x fork detection time, was 24 * 60 * 60 in billiecoin
...
/** Absolute maximum transaction fee (in finneas) used by wallet and mempool (rejects high fee in sendrawtransaction) */
...
bool CheckBilliecoinInputs(const CTransaction& tx, CValidationState &state, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, const CBlock& block, bool bSanity = false);
...
// BILLIECOIN add it here so chain.cpp can access
