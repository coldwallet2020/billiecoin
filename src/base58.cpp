...
namespace
{
class CBilliecoinAddressVisitor : public boost::static_visitor<bool>
{
private:
	CBilliecoinAddress* addr;
	// BILLIECOIN support old sys
	CChainParams::AddressType nSysVer;
public:
	CBilliecoinAddressVisitor(CBilliecoinAddress* addrIn) : addr(addrIn) {}
	CBilliecoinAddressVisitor(CBilliecoinAddress* addrIn, CChainParams::AddressType nSysVer) : nSysVer(nSysVer), addr(addrIn) {}
...
CBilliecoinAddress::CBilliecoinAddress() {
}
// BILLIECOIN support old sys
CBilliecoinAddress::CBilliecoinAddress(const CTxDestination &dest, CChainParams::AddressType sysVer) {
	Set(dest, sysVer);
}
CBilliecoinAddress::CBilliecoinAddress(const std::string& strAddress) {
	SetString(strAddress);
}
CBilliecoinAddress::CBilliecoinAddress(const char* pszAddress) {
	SetString(pszAddress);
}
// BILLIECOIN support old sys
bool CBilliecoinAddress::Set(const CKeyID& id, CChainParams::AddressType sysVer)
{
...
bool CBilliecoinAddress::Set(const CScriptID& id, CChainParams::AddressType sysVer)
{
	if (sysVer == CChainParams::ADDRESS_SYS)
...
// BILLIECOIN support old sys
bool CBilliecoinAddress::Set(const CTxDestination& dest, CChainParams::AddressType sysVer)
{
	return boost::apply_visitor(CBilliecoinAddressVisitor(this, sysVer), dest);
}

bool CBilliecoinAddress::IsValid() const
{
    return IsValid(Params());
}

bool CBilliecoinAddress::IsValid(const CChainParams& params) const
{
    bool fCorrectSize = vchData.size() == 20;
	// BILLIECOIN allow old BILLIECOIN address scheme
...
CTxDestination CBilliecoinAddress::Get() const
{
	if (!IsValid())
...
bool CBilliecoinAddress::GetIndexKey(uint160& hashBytes, int& type) const
{
    if (!IsValid()) {
        return false;
...
bool CBilliecoinAddress::GetKeyID(CKeyID& keyID) const
{
	// BILLIECOIN allow old BILLIECOIN address scheme
...
bool CBilliecoinAddress::IsScript() const
{
...
void CBilliecoinSecret::SetKey(const CKey& vchSecret)
{
...
bool CBilliecoinSecret::IsValid() const
{
...
bool CBilliecoinSecret::SetString(const char* pszSecret)
{
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CBilliecoinSecret::SetString(const std::string& strSecret)
{
