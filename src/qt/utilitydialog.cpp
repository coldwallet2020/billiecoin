    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (helpMode == about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  billiecoin-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_BILLIECOIN_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", BilliecoinGUI::DEFAULT_UIPLATFORM));
        }
...
All the Billiecoin in your wallet is comprised of different \"inputs\" which you can think of as separate, discrete coins.<br> \
...
These denominations are 0.1 BILCO, 1 BILCO, 10 BILCO and 100 BILCO -- sort of like the paper money you use every day.</li> \
...
For more info see <a href=\"https://billiecoin.green\">https://billiecoin.green</a> \
        "));
        ui->aboutMessage->setWordWrap(true);
        ui->helpMessage->setVisible(false);
        ui->aboutLogo->setVisible(false);
...
QWidget *ShutdownWindow::showShutdownWindow(BilliecoinGUI *window)
{
    if (!window)
        return nullptr;
