    ...
    platformStyle(_platformStyle)
{
    ...
    // see https://github.com/billiecoin/billiecoin/issues/5716
...
{
    GUIUtil::setClipboard(BilliecoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BilliecoinUnits::BILCO;
    ...
    l2->setText(BilliecoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(BilliecoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(BilliecoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l7->setText(fDust ? tr("yes") : tr("no"));                               // Dust
    l8->setText(BilliecoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0 && (coinControl->nMinimumTotalFee < nPayFee))
    {
    ...
    // how many finneas the estimated fee can vary per byte we guess wrong
    double dFeeVary;
    ...
    QString toolTip4 = tr("Can vary +/- %1 finnea(s) per input.").arg(dFeeVary);
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            {
                sAddress = QString::fromStdString(CBilliecoinAddress(outputAddress).ToString());

                // if listMode or change => show billiecoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, BilliecoinUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
            itemOutput->setToolTip(COLUMN_AMOUNT, BilliecoinUnits::format(nDisplayUnit, out.tx->tx->vout[out.i].nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, BilliecoinUnits::format(nDisplayUnit, nSum));
            itemWalletAddress->setToolTip(COLUMN_AMOUNT, BilliecoinUnits::format(nDisplayUnit, nSum));
