...
const std::string BilliecoinGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString BilliecoinGUI::DEFAULT_WALLET = "~Default";

BilliecoinGUI::BilliecoinGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void BilliecoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Billiecoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and billiecoin: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about Billiecoin Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Billiecoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Billiecoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a billiecoin: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Billiecoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BilliecoinGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void BilliecoinGUI::createToolBars()
{
#ifdef ENABLE_WALLET
    if(walletFrame)
    {
...
void BilliecoinGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool BilliecoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool BilliecoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BilliecoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BilliecoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void BilliecoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
...
void BilliecoinGUI::createIconMenu(QMenu *pmenu)
{
...
#ifndef Q_OS_MAC
void BilliecoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BilliecoinGUI::optionsClicked()
{
...
void BilliecoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BilliecoinGUI::showDebugWindow()
{
...
void BilliecoinGUI::showInfo()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void BilliecoinGUI::showConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void BilliecoinGUI::showGraph()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void BilliecoinGUI::showPeers()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void BilliecoinGUI::showRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void BilliecoinGUI::showConfEditor()
{
    GUIUtil::openConfigfile();
}

void BilliecoinGUI::showMNConfEditor()
{
    GUIUtil::openMNConfigfile();
}

void BilliecoinGUI::showBackups()
{
    GUIUtil::showBackups();
}

void BilliecoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

void BilliecoinGUI::showPrivateSendHelpClicked()
{
    if(!clientModel)
        return;
...
#ifdef ENABLE_WALLET
void BilliecoinGUI::openClicked()
{
...
void BilliecoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BilliecoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BilliecoinGUI::gotoMasternodePage()
{
    QSettings settings;
    if (!fLiteMode && settings.value("fShowMasternodesTab").toBool() && masternodeAction) {
...
void BilliecoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BilliecoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BilliecoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BilliecoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void BilliecoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Billiecoin network", "", count));
    } else {
...
void BilliecoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BilliecoinGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void BilliecoinGUI::updateHeadersSyncProgressLabel()
{
...
void BilliecoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
    ...
    if (walletFrame)
    {
        if(secs < 25*60) // 90*60 in billiecoin
        {
...
void BilliecoinGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    if(!clientModel)
        return;
...
void BilliecoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Billiecoin Core"); // default title
    ...
    // Append title to "Billiecoin Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
...
void BilliecoinGUI::changeEvent(QEvent *e)
{
...
void BilliecoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BilliecoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BilliecoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BilliecoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void BilliecoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BilliecoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BilliecoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BilliecoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BilliecoinGUI::setHDStatus(int hdEnabled)
{
    QString theme = GUIUtil::getThemeName();
...
void BilliecoinGUI::setEncryptionStatus(int status)
{
    QString theme = GUIUtil::getThemeName();
    switch(status)
    {
...
void BilliecoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BilliecoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BilliecoinGUI::detectShutdown()
{
...
void BilliecoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void BilliecoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void BilliecoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BilliecoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void BilliecoinGUI::subscribeToCoreSignals()
{
...
void BilliecoinGUI::unsubscribeFromCoreSignals()
{
...
void BilliecoinGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void BilliecoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<BilliecoinUnits::Unit> units = BilliecoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const BilliecoinUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(BilliecoinUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    Q_FOREACH(BilliecoinUnits::Unit u, BilliecoinUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(BilliecoinUnits::name(u)), this);
...
{
    setText(BilliecoinUnits::name(newUnits));
}
