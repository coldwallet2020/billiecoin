             ...
             CWalletScanState &wss, std::string& strType, std::string& strErr)
{
    try {
        ...
        if (strType == "name")
        {
            ...
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CBilliecoinAddress(strAddress).Get()].name;
        }
        else if (strType == "purpose")
        {
            std::string strAddress;
            ssKey >> strAddress;
            ssValue >> pwallet->mapAddressBook[CBilliecoinAddress(strAddress).Get()].purpose;
        }
        else if (strType == "tx")
        {
            uint256 hash;
			...
			{
				// BILLIECOIN
				if (wtx.tx->GetHash() != hash && wtx.tx->nVersion == BILLIECOIN_TX_VERSION)
					return true;
        ...
        else if (strType == "destdata")
        {
            ...
            ssKey >> strKey;
            ssValue >> strValue;
            if (!pwallet->LoadDestData(CBilliecoinAddress(strAddress).Get(), strKey, strValue))
            {
...
void ThreadFlushWalletDB()
{
    ...
    RenameThread("billiecoin-wallet");
