...
/** 
 * Fees smaller than this (in finneas) are considered zero fee (for transaction creation)
...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    {
        std::string strAddr = CBilliecoinAddress(CScriptID(redeemScript)).ToString();
...
void CWallet::AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, bool fIncludeZeroValue, AvailableCoinsType nCoinType, bool fUseInstantSend, bool fIncludeBilliecoinAliasBalances, bool fIncludeBilliecoinAliasOutputs) const
{
    vCoins.clear();

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
				// BILLIECOIN
				if (!fIncludeBilliecoinAliasBalances || !fIncludeBilliecoinAliasOutputs) {
					if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs && !coinControl->IsSelected(COutPoint((*it).first, i)))
						continue;
					// BILLIECOIN txs are unspendable by wallet unless using coincontrol(and the tx is selected)
					if (!coinControl || !coinControl->IsSelected(COutPoint((*it).first, i)))
					{
						...
						if (pcoin->tx->vout.size() >= i && ExtractDestination(pcoin->tx->vout[i].scriptPubKey, sysdestination))
						{
							...
							if (IsBilliecoinScript(pcoin->tx->vout[i].scriptPubKey, op, vvchArgs) && !fIncludeBilliecoinAliasOutputs)
								continue;
							CBilliecoinAddress address = CBilliecoinAddress(sysdestination);
							if (DoesAliasExist(address.ToString()) && !fIncludeBilliecoinAliasBalances)
								continue;
						}
					}
				}
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxDSIn>& vecTxDSInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax)
{
	...
	// BILLIECOIN include billiecoin alias balances
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable, bool fSkipUnconfirmed) const
{
    ...
    if (LogAcceptCategory("selectcoins")) {
        ...
        for (const auto& item : vecTallyRet)
            strMessage += strprintf("  %s %f\n", CBilliecoinAddress(item.txdest).ToString().c_str(), float(item.nAmount)/COIN);
...
bool CWallet::GetMasternodeOutpointAndKeys(COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet, const std::string& strTxHash, const std::string& strOutputIndex)
{
	...
	// BILLIECOIN include sys alias balances
...
bool CWallet::GetOutpointAndKeysFromOutput(const COutput& out, COutPoint& outpointRet, CPubKey& pubKeyRet, CKey& keyRet)
{
    ...
    CBilliecoinAddress address2(address1);
...
{
    std::vector<COutput> vCoins;
	// BILLIECOIN include alias balances
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 BILCO."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-billiecoin-address
                        CScript scriptChange;
...
bool CWallet::SetAddressBook(const CTxDestination& address, const std::string& strName, const std::string& strPurpose)
{
    ...
    if (!fFileBacked)
        return false;
    if (!strPurpose.empty() && !CWalletDB(strWalletFile).WritePurpose(CBilliecoinAddress(address).ToString(), strPurpose))
        return false;
    return CWalletDB(strWalletFile).WriteName(CBilliecoinAddress(address).ToString(), strName);
}
...
{
    {
        ...
        if(fFileBacked)
        {
            ...
            std::string strAddress = CBilliecoinAddress(address).ToString();
    ...
    if (!fFileBacked)
        return false;
    CWalletDB(strWalletFile).ErasePurpose(CBilliecoinAddress(address).ToString());
    return CWalletDB(strWalletFile).EraseName(CBilliecoinAddress(address).ToString());
}
...
bool CWallet::AddDestData(const CTxDestination &dest, const std::string &key, const std::string &value)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).WriteDestData(CBilliecoinAddress(dest).ToString(), key, value);
}
...
bool CWallet::EraseDestData(const CTxDestination &dest, const std::string &key)
{
    ...
    if (!fFileBacked)
        return true;
    return CWalletDB(strWalletFile).EraseDestData(CBilliecoinAddress(dest).ToString(), key);
}
