...
// Billiecoin This is not a CORS check. Only check if OPTIONS are
...
static bool checkPreflight(HTTPRequest* req)
{
    ...
    // Billiecoin Keep origin so as not to have to set Access-Control-Allow-Origin
    // to *
    ...
    // unbounded.
    // Billiecoin We are only satifying the OPTIONS request in this method in order to
    ...
    if (req->GetRequestMethod() == HTTPRequest::OPTIONS) {
        ...
        // Billiecoin as we are only handling OPTIONS there is
