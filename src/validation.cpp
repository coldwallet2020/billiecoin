...
#if defined(NDEBUG)
# error "Billiecoin Core cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Billiecoin Signed Message:\n";
...
// BILLIECOIN
bool CheckBilliecoinInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache &inputs, bool fJustCheck, int nHeight, const CBlock& block, bool bSanity)
{
	...
	if (block.vtx.empty() && tx.nVersion == BILLIECOIN_TX_VERSION) {
		...
		if (good)
		{
			...
			return state.DoS(100, false, REJECT_INVALID, "billiecoin-inputs-error");
		...
		if (CreateGraphFromVTX(nHeight, sortedBlock.vtx, graph, vertices, mapTxIndex)) {
			...
			if (!sortedBlock.vtx.empty()) {
				if (!DAGTopologicalSort(sortedBlock.vtx, conflictedIndexes, graph, mapTxIndex)) {
					if (fDebug)
						LogPrintf("CheckBilliecoinInputs: Toposort failed");
					return true;
				}
			}
		}
		...
		nFlushIndexBlocks++;
		if ((nFlushIndexBlocks % 200) == 0 && !FlushBilliecoinDBs())
			return state.DoS(0, false, REJECT_INVALID, "Failed to flush billiecoin databases");
	}
	return true;
}
                              ...
                              std::vector<COutPoint>& coins_to_uncache, bool fDryRun, bool bMultiThreaded)
{
		...
		BOOST_FOREACH(const CTxIn &txin, tx.vin)
		{
			...
			if (itConflicting != pool.mapNextTx.end())
			{
				...
				if (!setConflicts.count(ptxConflicting->GetHash()))
				{
					...
					// BILLIECOIN txs are not replacable
					else if (ptxConflicting->nVersion == BILLIECOIN_TX_VERSION) {
						return state.DoS(0, error("AcceptToMemoryPool : Billiecoin Transaction %s conflicts with Transaction request %s",
							hash.ToString(), ptxConflicting->GetHash().ToString()),
...

	{
		...
		if (!bMultiThreaded) {
			...
			if (!control.Wait())
				return false;
			if (!CheckBilliecoinInputs(tx, state, view, true, chainActive.Height(), CBlock())) {
				return false;
			}
		}
		...
		if (bMultiThreaded && threadpool != NULL)
		{
			...
			static int totalBilliecoinCheckCount = 0;
			static int totalBilliecoinCheckMicros = 0;
			...
			std::packaged_task<void()> task([&pool, ptx, hash, coins_to_uncache, hashCacheEntry, vChecks]() {
				...
				int thisBilliecoinCheckCount = 0;
				int thisCheckMicros = 0;
				int thisBilliecoinCheckMicros = 0;
				...
				if (isCheckPassing)
				{
					int64_t billiecoinCheckTime;
					if (fLogThreadpool) {
						billiecoinCheckTime = GetTimeMicros();
						thisBilliecoinCheckCount += 1;
					}
					if (!CheckBilliecoinInputs(txIn, validationState, coinsViewCache, true, chainActive.Height(), CBlock()))
					{
						nLastMultithreadMempoolFailure = GetTime();
						LOCK2(cs_main, mempool.cs);
						LogPrint("mempool", "%s: %s\n", "CheckBilliecoinInputs Error", hash.ToString());
						BOOST_FOREACH(const COutPoint& hashTx, coins_to_uncache)
							pcoinsTip->Uncache(hashTx);
						pool.removeRecursive(txIn, MemPoolRemovalReason::UNKNOWN);
						pool.ClearPrioritisation(hash);
					...
					if (fLogThreadpool) {
						// how long did we run CheckBilliecoinInputs()'s
						thisBilliecoinCheckMicros = GetTimeMicros() - billiecoinCheckTime;
					}
				}
				if (fLogThreadpool) {
					// metrics
					...
					totalBilliecoinCheckCount += thisBilliecoinCheckCount;
					totalBilliecoinCheckMicros += thisBilliecoinCheckMicros;
			...
			if (fLogThreadpool && totalCheckCount > 0 && totalBilliecoinCheckCount > 0 && totalExecutionCount > 0 && (!fUnitTest || (fUnitTest && totalExecutionCount % 100 == 0))) {
				int avgCheckMicros = totalCheckMicros / totalCheckCount;
				int avgBilliecoinCheckMicros = totalBilliecoinCheckMicros / totalBilliecoinCheckCount;
				...
				const std::string &messageInternals = "THREADPOOL::%s:Signature check internals - check(%d): total %lld, avg %lld microseconds, billiecoin(%d): total %lld, avg %lld microseconds\n";
				...
				LogPrint("threadpool", messageInternals, hash.ToString(), totalCheckCount, totalCheckMicros, avgCheckMicros, totalBilliecoinCheckCount, totalBilliecoinCheckMicros, avgBilliecoinCheckMicros);
...
{
	// BILLIECOIN if its been less 60 seconds since the last MT mempool verification failure then fallback to single threaded
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams, CAmount &nTotalRewardWithMasternodes, bool fSuperblockPartOnly, bool fMasternodePartOnly, unsigned int nStartHeight)
{
	...
	if (nHeight == 1)
	{
		...
		// BILLIECOIN 3 snapshot
		nTotalRewardWithMasternodes = 12000000 * COIN;
	...
	// yearly decline of production by 5% per year, projected ~300M coins max by year 2067+.
        ...
        if (fAddressIndex) {
            ...
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if (out.scriptPubKey.IsPayToScriptHash()) {
					...
					if (RemoveBilliecoinScript(out.scriptPubKey, scriptPubKeyOut))
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (fAddressIndex) {
                    ...
                    if (prevout.scriptPubKey.IsPayToScriptHash()) {
						...
						CScript scriptOut;
						CScript scriptPubKeyOut;
						if (RemoveBilliecoinScript(prevout.scriptPubKey, scriptPubKeyOut))
							scriptOut = scriptPubKeyOut;
						else
...
void ThreadScriptCheck() {
    RenameThread("billiecoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
	...
	if (!CheckBilliecoinInputs(*block.vtx[0], state, viewOld, fJustCheck, pindex->nHeight, block))
		return error("ConnectBlock(): CheckBilliecoinInputs on block %s failed\n",
			block.GetHash().ToString());
    int64_t nTime3 = GetTimeMicros(); nTimeConnect += nTime3 - nTime2;
    LogPrint("bench", "      - Connect %u transactions: %.2fms (%.3fms/tx, %.3fms/txin) [%.2fs]\n", (unsigned)block.vtx.size(), 0.001 * (nTime3 - nTime2), 0.001 * (nTime3 - nTime2) / block.vtx.size(), nInputs <= 1 ? 0 : 0.001 * (nTime3 - nTime2) / (nInputs-1), nTimeConnect * 0.000001);

    // BILLIECOIN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
	...
	if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, nFees, nTotalRewardWithMasternodes)) {
		...
		return state.DoS(0, error("ConnectBlock(BILCO): couldn't find masternode or superblock payments"),
			REJECT_INVALID, "bad-cb-payee");
	}

    std::string strError = "";
    if (!IsBlockValueValid(block, pindex->nHeight, nTotalRewardWithMasternodes, nFees, strError)) {
        return state.DoS(0, error("ConnectBlock(BILCO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    // END BILLIECOIN
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    if (fDoFullFlush) {
		...
		// BILLIECOIN
		if (!FlushBilliecoinDBs())
			return AbortNode(state, "Failed to flush billiecoin databases");
        nLastFlush = nNow;
    }
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
			// BILLIECOIN, skip auxpow bit 8 or dummy bit 28
			if (bit == 8 || bit == 28)
				continue;
...
{
	// BILLIECOIN check auxpow first then fallback to normal check
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // BILCO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(BILCO): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // BILLIECOIN check for version 2, 3 and 4 upgrades
