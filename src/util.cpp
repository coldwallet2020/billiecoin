...
const char * const BILLIECOIN_CONF_FILENAME = "billiecoin.conf";
const char * const BILLIECOIN_PID_FILENAME = "billiecoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "billiecoin" is a composite category enabling all Billiecoin-related debug output
                if(ptrCategory->count(std::string("billiecoin"))) {
...
#else
    const char* pszModule = "billiecoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BilliecoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BilliecoinCore
    // Mac: ~/Library/Application Support/BilliecoinCore
    // Unix: ~/.billiecoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BilliecoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BilliecoinCore";
#else
    // Unix
    return pathRet / ".billiecoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty billiecoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override billiecoin.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", BILLIECOIN_PID_FILENAME));
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Billiecoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Billiecoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2009, nEndYear) + "The Billiecoin Core developers";
    }
